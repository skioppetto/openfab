...
public class AbstractEquipmentTreeProviderTest {
	...
	public void testTreeRoot() {
		Assert.assertEquals(provider.getRoot()
				.getID(), "root");
	}
	...
	public void testChildren() {
		...
		AbstractEquipmentTreeNode root = provider
				.asTree();
		...
		root.getChildren().forEach(
				child -> chidldrenIDs.add(child.getEquipment().getID()));
		...
		Optional<AbstractEquipmentTreeNode> node1 = root.getChildren().stream()
				.filter(child -> "node1".equals(child.getEquipment().getID()))
				.findFirst();
		...
		Optional<AbstractEquipmentTreeNode> node3 = node1.get().getChildren()
				.stream()
				.filter(child -> "node3".equals(child.getEquipment().getID()))
				.findFirst();
		...
		Optional<AbstractEquipmentTreeNode> node4 = node1.get().getChildren()
				.stream()
				.filter(child -> "node4".equals(child.getEquipment().getID()))
				.findFirst();
		...
		Optional<AbstractEquipmentTreeNode> node2 = root.getChildren().stream()
				.filter(child -> "node2".equals(child.getEquipment().getID()))
				.findFirst();
		...
		Optional<AbstractEquipmentTreeNode> node5 = node2.get().getChildren()
				.stream()
				.filter(child -> "node5".equals(child.getEquipment().getID()))
				.findFirst();
